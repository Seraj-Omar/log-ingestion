...
export class IngestionBatcher {
    ...
    public tryIngest(
        ...
        if(
            !this.accepting||
            this.inFlightRequests+1>this.options.maxInFlightRequests||
            this.inFlightLogs+logs.length>this.options.maxInFlightLogs||
    ...
    private takeNextBatch():IngestionJob[]{
        ...
        while(this.hasQueuedJobs()){
            ...
            if(
                jobs.length>0&&
                batchLogs+job.logs.length>this.options.batchSize
            ){
                break;
            }
